...
cat("\n[步骤 1] 读取表达矩阵...\n")
exp_raw <- read.delim("GSE148036_PRJNA609278count_matrix.txt", check.names = FALSE)
gene_col <- as.character(exp_raw[, 1])
exp_raw <- exp_raw[, -1, drop = FALSE]

cat("原始维度: ", nrow(exp_raw), " x ", ncol(exp_raw), "\n", sep = "")

# ENSGxxxx(.xx) -> ENSGxxxx
ids <- gsub("\\..*$", "", gene_col)

cat("\n[步骤 2] 转换ID: ENSG -> SYMBOL...\n")
symbols <- mapIds(
  org.Hs.eg.db,
  keys = ids,
  keytype = "ENSEMBL",
