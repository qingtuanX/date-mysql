...
res_pattern <- "^GSE148036_limma_test_result\\..+_vs_Others\\.txt$"
out_dir <- "plots"
...
extract_comp <- function(path) {
  base <- basename(path)
  sub("^GSE148036_limma_test_result\\.(.+)\\.txt$", "\\1", base)
}
...
for (f in res_files) {
  ...
  tag <- safe_name(comp)
  volcano_png <- file.path(out_dir, paste0("GSE148036_volcano_", tag, ".png"))
  heatmap_png <- file.path(out_dir, paste0("GSE148036_heatmap_", tag, ".png"))
